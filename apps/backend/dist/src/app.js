...
export function createApp() {
    ...
    app.get("/v1/search", async (req, reply) => {
        try {
            const { q, lat, lng, onlyTourism, minRating, minReviews, sort, theme, region } = req.query ?? {};
            ...
            arr = arr.filter((p) => (p.rating ?? 0) >= mr && (p.userRatingCount ?? 0) >= mv);
            // Apply sorting
            const s = String(sort ?? "score");
            try {
                if (s === "score")
                    arr.sort((a, b) => scorePlace(b) - scorePlace(a));
                else if (s === "rating")
    ...
    app.post("/v1/itineraries/:id/items", async (req, reply) => {
        try {
            ...
            const { placeId, name, address, lat, lng, day, startTime, endTime, memo } = req.body || {};
            if (!placeId || !name) {
